...
export function parseLineSuffix(raw: string): { path: string; lines?: Mention['lines'] } {
  const match = /^(.*):(\d+)(?:-(\d+))?$/.exec(raw)
  if (match === null) return { path: raw }
  const start = Number(match[2])
  if (!Number.isSafeInteger(start) || start < 1) return { path: raw }
  const endRaw = match[3]
  if (endRaw === undefined) return { path: match[1] as string, lines: { start, end: start } }
  const end = Number(endRaw)
  if (!Number.isSafeInteger(end) || end < start) return { path: raw }
  return { path: match[1] as string, lines: { start, end } }
}
 ...
 */
export function scanMentions(text: string): readonly Mention[] {
...
async function readContent(
  ...
  try {
    ...
    if (head.subarray(0, headRead).includes(0)) return undefined
...
function referenceForm(mention: Mention, content?: string): string {
  ...
  if (content === undefined) return `<file-reference ${attrsText} />`
...
export async function expandMentions(
  ...
  signal: AbortSignal,
  options: MentionOptions = { includeContent: true, maxBytes: 20000 },
): Promise<UserMessage[]> {
  ...
  for (const token of tokens) {
    ...
    const resolved: Mention = token.lines !== undefined
